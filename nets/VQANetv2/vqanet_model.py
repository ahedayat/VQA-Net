...
class VQANet(nn.Module):
    def __init__(self, rnn_type,rnn_hidden_size, question_word_size, rnn_num_layers=1, rnn_bidirectional=False, rnn_dropout=False, cnn_output_size=512):
        ...
        self.rnn_linear_layer = nn.Sequential(
                                        nn.Linear(question_word_size+cnn_output_size, question_word_size, bias=True),
                                        nn.ReLU()
                                       )
    ...
    def forward(self, image_question_answer_distractors, train=False):
        ...
        image_rep_expanded = image_rep.expand( encoded_question.size()[0], image_rep.size()[1], image_rep.size()[2] )

        encoded_question = torch.cat( (encoded_question, image_rep_expanded), dim=2)
        encoded_question = self.rnn_linear_layer( encoded_question )
    ...
    def initialize_rnn(self, image_rep):
        size = ( image_rep.size()[0], image_rep.size()[1], self.rnn_hidden_size )

        zero_tensor = torch.zeros( size )
        if image_rep.device.type=='cuda':
            zero_tensor = zero_tensor.cuda()

        self.rnn_h = self.init_layer_h(zero_tensor)
        ...
        if self.rnn_type=='LSTM':
            self.rnn_c = self.init_layer_c(zero_tensor)
